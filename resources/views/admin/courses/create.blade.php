...
<body class="bg-gradient-to-br from-teal-50 via-white to-teal-100 min-h-screen">
    ...
    @endif
        ...
        <div class="bg-white rounded-2xl shadow-xl overflow-hidden border border-teal-100">
            ...
            <div class="bg-gradient-to-r from-teal-600 to-teal-700 p-6">
                <h1 class="text-3xl font-bold text-white tracking-wide">Create Course</h1>
            </div>
            ...
            <form action="{{ route('courses.store') }}" method="POST" class="p-6 space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="course_name"
                               class="w-full px-4 py-2 rounded-lg border border-teal-300 focus:border-teal-500 focus:ring-2 focus:ring-teal-200 transition-all duration-300" value="{{ old('course_name', $course->course_name ?? '') }}"
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="course_code"
                               class="w-full px-4 py-2 rounded-lg border border-teal-300 focus:border-teal-500 focus:ring-2 focus:ring-teal-200 transition-all duration-300" value="{{ old('course_code', $course->course_code ?? '') }}"
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" name="credit_hours"
                               class="w-full px-4 py-2 rounded-lg border border-teal-300 focus:border-teal-500 focus:ring-2 focus:ring-teal-200 transition-all duration-300" value="{{ old('credit_hours', $course->credit_hours ?? '') }}"
                ...
                <div class="space-y-6">
                    ...
                    <div class="form-group">
                        ...
                        <select name="prerequisite_ids[]" id="prerequisites" multiple class="w-full px-4 py-2 rounded-lg border border-teal-300 focus:border-teal-500 focus:ring-2 focus:ring-teal-200 transition-all duration-300">
    ...
    <script>
        $(document).ready(function() {
            ...
            $('#courses').change(function() {
                var courseId = $(this).val(); // Get selected course ID
                if (courseId) {
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            console.log('Prerequisites:', data.prerequisites); // Log the prerequisites specifically
                            // Check if prerequisites exist
                            if (data.prerequisites && Array.isArray(data.prerequisites)) {
                                console.log('Prerequisites array:', data.prerequisites); // Log the prerequisites array
                                if (data.prerequisites.length > 0) {
                                    var prerequisitesList = '<ul>';
                                    data.prerequisites.forEach(function(prerequisite) {
                                        prerequisitesList += '<li>' + prerequisite.course_name + '</li>';
                                    });
                                    prerequisitesList += '</ul>';
                                    $('#prerequisites-container').html(prerequisitesList);
                                } else {
                                    $('#prerequisites-container').html('<p>No prerequisites required.</p>');
                                }
                            } else {
                                $('#prerequisites-container').html('<p>No prerequisites found.</p>');
                            }
                        },
                        error: function(xhr, status, error) {
                            console.error('AJAX Error:', status, error); // Log the error details
                            $('#prerequisites-container').html('<p>Error fetching prerequisites.</p>');
                        }
                    });
                }
            });
    ...
    <div id="prerequisites-container" class="mt-4"></div>
</body>
</html>
