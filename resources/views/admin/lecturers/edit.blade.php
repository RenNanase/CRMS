    ...
    <div class="container mx-auto px-4 py-8 max-w-6xl">
        ...
        <div class="bg-white shadow-xl rounded-2xl overflow-hidden">
            ...
            <div class="bg-teal-600 text-white px-8 py-6">
                <h2 class="text-3xl font-bold">Edit Lecturer</h2>
                <p class="mt-2 text-teal-100">Update the lecturer's information</p>
            </div>

            <!-- Form Section -->
            <div class="p-8">
                <form action="{{ route('admin.lecturers.update', $lecturer->id) }}" method="POST" class="space-y-8">
                    @csrf
                    @method('PUT')
                            ...
                            <div class="bg-teal-50 p-6 rounded-xl">
                                ...
                                <div class="space-y-6">
                                    <div>
                                        ...
                                        <input type="text" name="name" id="name" required value="{{ old('name', $lecturer->name) }}"
                                    ...
                                    <div>
                                        ...
                                        <input type="email" name="email" id="email" required value="{{ old('email', $lecturer->email) }}"
                                               class="w-full h-12 px-4 rounded-lg border-2 border-teal-200 shadow-sm focus:border-teal-500 focus:ring focus:ring-teal-200 focus:ring-opacity-50 transition duration-200">
                                    </div>

                                    <div>
                                        <label for="phone" class="block text-sm font-medium text-gray-700 mb-2">Phone Number</label>
                                        <input type="text" name="phone" id="phone" required value="{{ old('phone', $lecturer->phone) }}"
                                               class="w-full h-12 px-4 rounded-lg border-2 border-teal-200 shadow-sm focus:border-teal-500 focus:ring focus:ring-teal-200 focus:ring-opacity-50 transition duration-200">
                                    </div>
                                </div>
                            </div>
                                            ...
                                            <div class="space-y-2">
                                                ...
                                                <label class="flex items-center p-2 hover:bg-teal-50 rounded-lg cursor-pointer group transition-colors duration-150">
                                                    <input type="checkbox" 
                                                           name="course_ids[]" 
                                                           value="{{ $course->id }}"
                                                           {{ in_array($course->id, old('course_ids', $lecturer->courses->pluck('id')->toArray())) ? 'checked' : '' }}
