    ...
    <style>
        ...
        * {
        ...
        /* Modal Styles */
            ...
            background-color: #0d9488; /* Changed to soft teal */
            padding: 1.5rem;
            ...
            .modal-title {
            font-size: 1.25rem;
            color: #befffa; /* Darker teal for better contrast */
            margin: 0;
            font-weight: 600;
            }
...
<!-- Programme Structure Card -->
<div class="col-12">
    <div class="card">
        <div class="card-body">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h5 class="card-title mb-0">Programme Structure</h5>
            </div>
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        <div class="p-3 bg-teal-50 rounded-lg">
                            <h6 class="text-teal-700 mb-1">
                                <i class="fas fa-graduation-cap me-2"></i>Program
                            </h6>
                            <p class="mb-0 text-teal-800">{{ $student->program->name ?? 'Not Available' }} ({{
                                $student->program->code ?? 'N/A' }})</p>
                        </div>
                    </div>
                    <div class="col-md-6">
                        <div class="p-3 bg-teal-50 rounded-lg">
                            ...
                            <p class="mb-0 text-teal-800">{{ $student->faculty->faculty_name ?? 'Not Available' }}</p>
                        </div>
                    </div>
                </div>
            </div>

            @if(isset($programStructure) && $programStructure->pdf_path)
            <div class="text-center py-4">
                <div class="mb-3">
                    <i class="fas fa-file-pdf text-teal-500" style="font-size: 3rem;"></i>
                </div>
                <h6 class="text-teal-700 mb-3">Programme Structure Document</h6>
                <div class="btn-group">
                    <button type="button" class="btn btn-outline-teal me-2" data-bs-toggle="modal"
                        data-bs-target="#pdfModal">
                        <i class="fas fa-eye me-1"></i> View Document
                    </button>
                    <a href="{{ route('student.program-structure.download') }}" class="btn btn-outline-teal">
            ...
            <div class="text-center py-4">
                ...
                <h6 class="text-teal-700">Program Structure Not Available</h6>
                <p class="text-muted">Please contact your academic advisor for more information.</p>
            </div>
            @endif
        </div>
    </div>
    ...
    <div class="modal-dialog modal-xl modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header bg-teal-600 text-white">
                <h5 class="modal-title" id="pdfModalLabel">Programme Structure</h5>
                <button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"
                    aria-label="Close"></button>
            </div>
            <div class="modal-body p-0">
                @if(isset($programStructure) && $programStructure->pdf_path)
                <iframe src="{{ Storage::url($programStructure->pdf_path) }}#toolbar=0" class="w-100"
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (viewPdfBtn) {
                viewPdfBtn.addEventListener('click', function(e) {
                    e.preventDefault();
                    const pdfUrl = "{{ $programStructure->pdf_path ?? '' }}";
                    if (pdfUrl) {
                        window.open("{{ Storage::url('') }}" + pdfUrl, '_blank');
                    } else {
            ...
            if (downloadPdfBtn) {
                downloadPdfBtn.addEventListener('click', function(e) {
                    e.preventDefault();
                    const downloadUrl = "{{ route('student.program-structure.download') }}";
