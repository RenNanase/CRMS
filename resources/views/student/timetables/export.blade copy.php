...
<body class="bg-white">
    <div class="max-w-7xl mx-auto p-6 print:p-4">
        ...
        <div class="mb-8 text-center">
            <h1 class="text-3xl font-bold text-gray-800">Student Timetable</h1>
                ...
                <table class="w-full border-collapse bg-white">
                    <thead>
                        <tr class="divide-x divide-gray-200">
                            <th class="sticky top-0 px-4 py-3 bg-gray-50 text-left text-sm font-semibold text-gray-700 border-b border-gray-200 min-w-[100px]">
                                Time
                            </th>
                            @foreach(['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday'] as $day)
                                <th class="sticky top-0 px-4 py-3 bg-gray-50 text-left text-sm font-semibold text-gray-700 border-b border-gray-200 min-w-[200px]">
                                    {{ $day }}
                                </th>
                            @endforeach
                        </tr>
                    ...
                    <tbody class="divide-y divide-gray-200">
                        ...
                        @for($hour = $startHour; $hour <= $endHour; $hour++)
                            @foreach(['00', '30'] as $minute)
                                ...
                                @endphp
                                <tr class="divide-x divide-gray-200 hover:bg-gray-50 transition-colors duration-150">
                                    <td class="px-4 py-3 text-sm text-gray-600 font-medium border-r border-gray-200 whitespace-nowrap">
                                        {{ $currentTime }} - {{ $nextTime }}
                                    </td>

                                    @foreach(['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday'] as $day)
                                        <td class="px-3 py-2 relative min-h-[80px] border-r border-gray-200">
